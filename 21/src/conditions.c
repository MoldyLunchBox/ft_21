...
/*   Updated: 2021/03/08 17:23:47 by amya             ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int		is_printable(char *fd)
{
	...
	while(fd[i])
	{
		if ((( int )fd[i] >= 32 && (int )fd[i] < 127 )|| (int )fd[i] == 9 || (int )fd[i] == 10)
		i++;
		else
			return 0;
	}
...
int		is_not_button(char *fd)
 {
	 if ( *((int *)fd) == COPY ||*((int *)fd) == ALT_UP||*((int *)fd) == ALT_DOWN|| *((int *)fd) == CUT || *((int *)fd) == PASTE || *((int *)fd) == EXIT ||*((int *)fd) == SELECT || *((int *)fd) == ESC || *((int *)fd) == KEY_UP || *((int *)fd) == KEY_DOWN ||
